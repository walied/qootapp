    ...
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, fontFamily: lang === "ar" ? "'Alexandria',sans-serif" : "'Inter',sans-serif", direction: lang === "ar" ? "rtl" : "ltr" }}>
      <div className="fu" style={{ width: "100%", maxWidth: 400, background: C.card, border: `1px solid ${C.border}`, borderRadius: 20, padding: "32px 24px", textAlign: "center" }}>
        ...
        <div style={{ fontSize: 20, fontWeight: 700, color: C.text, marginBottom: 8 }}>
          {isSignUp ? (lang === "ar" ? "إنشاء حساب جديد" : "Create Account") : (lang === "ar" ? "تسجيل الدخول" : "Sign In")}
        </div>

        {successMessage && (
          <div style={{ background: `${C.green}18`, border: `1px solid ${C.green}44`, borderRadius: 10, padding: "8px 12px", marginBottom: 16, color: C.green, fontSize: 13 }}>
            {successMessage}
          </div>
        )}
        ...
        <form onSubmit={handleEmailAuth}>
          ...
          >
            {loading ? (lang === "ar" ? "جاري..." : "Loading...") : (isSignUp ? (lang === "ar" ? "إنشاء حساب" : "Sign Up") : (lang === "ar" ? "تسجيل الدخول" : "Sign In"))}
          </button>
        </form>

        <button
          onClick={() => { setIsSignUp(!isSignUp); setError(""); setSuccessMessage(""); setPassword(""); }}
          style={{ background: "none", border: "none", color: C.teal, cursor: "pointer", fontSize: 13, marginBottom: 16, textDecoration: "underline" }}
        >
          {isSignUp
            ? (lang === "ar" ? "لديك حساب؟ سجل الدخول" : "Already have an account? Sign in")
            : (lang === "ar" ? "ليس لديك حساب؟ أنشئ حساباً" : "No account? Create one")}
        </button>
        ...
        <button onClick={() => setScreen("landing")} style={{ background: "none", border: "none", color: C.muted, fontSize: 13, cursor: "pointer", marginTop: 16, textDecoration: "underline" }}>
