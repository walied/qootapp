    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: lang === "ar" ? "'Alexandria',sans-serif" : "'Inter',sans-serif", direction: lang === "ar" ? "rtl" : "ltr", overflowX: "hidden" }}>
      ...
      <div style={{ padding: "18px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${C.border}`, flexWrap: "wrap", gap: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          {role !== "customer" && (
            <span style={{ background: role === "admin" ? C.purple : C.teal, color: "#fff", borderRadius: 20, padding: "4px 12px", fontSize: 12, fontWeight: 600 }}>
              {role === "admin" ? (lang === "ar" ? "مدير" : "Admin") : (lang === "ar" ? "متخصص" : "Specialist")}
            </span>
          )}
          <button onClick={() => setLang(lang === "ar" ? "en" : "ar")} style={{ background: C.cardLight, border: `1px solid ${C.border}`, color: C.text, borderRadius: 8, padding: "6px 14px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
      ...
      <div style={{ maxWidth: 660, margin: "0 auto", padding: "56px 20px 40px", textAlign: "center" }}>
        ...
        {/* ✅ الزر المعدل: يوجه إلى AuthScreen بدلاً من QuizScreen مباشرة */}
        <button
          className="fu4"
      ...
      {/* Features & Steps – يبقى كما هو دون تغيير */}
        ...
        {[["01", C.teal], ["02", C.amber], ["03", C.purple], ["04", C.green]].map(([num, color], i) => (
          <div key={num} style={{ display: "flex", gap: 16, marginBottom: i < 3 ? 22 : 0 }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
              <div style={{ width: 40, height: 40, background: `${color}18`, border: `2px solid ${color}55`, borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color, flexShrink: 0 }}>{num}</div>
