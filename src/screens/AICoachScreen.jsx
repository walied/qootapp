...
const AICoachScreen = () => {
  ...
  const handleOnboardingAnswer = async (answer) => {
    ...
    await supabase.from('conversations').insert(userMsg);
    ...
    if (newStep >= steps.length) {
      ...
      await supabase.from('conversations').insert(loadingMsg);
      await generatePlan();
      return true;
    }
  ...
  const sendMessage = async (text, imageUrl = null) => {
    ...
    try {
      ...
      const errorReply = {
        role: 'assistant',
        content: lang === 'ar' ? 'عذراً، حدث خطأ في الاتصال. تأكد من مفتاح API.' : 'Sorry, connection error. Check API key.',
        created_at: new Date().toISOString(),
        user_id: user.id
      };
