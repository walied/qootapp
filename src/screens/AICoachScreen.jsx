...
const AICoachScreen = () => {
  ...
  // Onboarding steps (matches your existing user_profiles fields)
  const steps = [
    ...
    { key: 'health_conditions', questionAr: 'هل تعاني من أي أمراض مزمنة؟', questionEn: 'Any chronic diseases?', type: 'text' },
    { key: 'country', questionAr: 'ما هي بلد إقامتك؟', questionEn: 'What is your country?', type: 'text' },
    { key: 'diet_type', questionAr: 'نظامك الغذائي؟ (نباتي/كيتو/متوسطي/لا شيء)', questionEn: 'Diet type? (vegetarian/keto/mediterranean/none)', type: 'text' },
    { key: 'allergies', questionAr: 'هل لديك حساسيات غذائية؟', questionEn: 'Any allergies?', type: 'text' },
    { key: 'medications', questionAr: 'هل تتناول أدوية تؤثر على الوزن؟', questionEn: 'Medications affecting weight?', type: 'text' },
    { key: 'eating_out', questionAr: 'كم مرة تأكل خارج المنزل أسبوعياً؟ (0-1,2-3,4-5,أكثر من 5)', questionEn: 'Eating out frequency per week?', type: 'text' },
    { key: 'whatsapp', questionAr: 'رقم واتسابك (مثال: +96550123456)', questionEn: 'WhatsApp number (e.g., +96550123456)', type: 'text' }
  ];
  ...
  const sendMessage = async (text, imageUrl = null) => {
    ...
    setIsLoading(true);
    try {
      // Call DeepSeek API directly (or via an edge function)
      ...
      await supabase.from('conversations').insert([userMessage, assistantMessage]);
    } catch (error) {
  ...
  const analyzeMealImage = async (file, text) => {
    // Placeholder for meal image analysis (you can integrate Gemini or GPT-4 Vision)
    // For now, return a simple analysis.
    return "📊 التقدير التقريبي: 400 سعرة، 25g بروتين، 45g كارب، 15g دهون. وجبة متوازنة.";
  };
  ...
  const handleSendMessage = async () => {
    ...
    if (imageFile) {
      ...
      imageUrl = publicUrl;
      analysisText = await analyzeMealImage(imageFile, inputText);
      setImageFile(null);
      setImagePreview(null);
      if (fileInputRef.current) fileInputRef.current.value = '';
    }
    const finalText = inputText || (imageUrl ? `📷 تحليل الوجبة: ${analysisText}` : '');
