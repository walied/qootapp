...
const SidebarPlan = ({ isOpen, onClose, plan, userMetrics, lang, weekProgress }) => {
  ...
  // تحديد اليوم الحالي (يبدأ من 1)
  const currentDayIndex = weekProgress?.currentDay ? weekProgress.currentDay - 1 : 0;
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, width: '320px', maxWidth: '85%', height: '100vh', background: C.bg, boxShadow: '-4px 0 12px rgba(0,0,0,0.15)', zIndex: 1000, overflowY: 'auto', padding: '20px' }}>
        ...
        {/* Today's Plan – only current day */}
        {currentDayPlan ? (
          <div>
            <h3 style={{ fontSize: '16px', fontWeight: '600', color: C.text, marginBottom: '16px' }}>
              {lang === 'ar' ? `خطة اليوم (اليوم ${weekProgress?.currentDay || 1})` : `Today's Plan (Day ${weekProgress?.currentDay || 1})`}
            </h3>
            <div style={{ background: C.card, borderRadius: '12px', padding: '14px', marginBottom: '12px' }}>
              <div style={{ fontWeight: '600', color: C.text, marginBottom: '10px' }}>
                {currentDayPlan.day}
              </div>
              {['breakfast', 'lunch', 'dinner', 'snack'].map(meal => {
                const m = currentDayPlan[meal];
                ...
                return (
                  <div key={meal} style={{ marginBottom: '8px' }}>
                    <div style={{ fontSize: '12px', color: C.muted }}>{labels[meal]}</div>
                    <div style={{ fontSize: '14px', color: C.text }}>{m.options.join('، ')}</div>
                    {m.macros && <div style={{ fontSize: '12px', color: C.teal }}>{m.macros}</div>}
                  </div>
                );
              })}
            </div>
            <p style={{ fontSize: '12px', color: C.muted, textAlign: 'center' }}>
              {lang === 'ar' ? 'لرؤية الخطة كاملة، اسأل المساعد "أرني خطة الأسبوع"' : 'To see full week, ask "Show me weekly plan"'}
            </p>
          </div>
        ) : (
